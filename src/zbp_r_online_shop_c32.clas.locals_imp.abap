CLASS lsc_zbp_r_online_shop_c32 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
    METHODS save_modified REDEFINITION.
ENDCLASS.

CLASS lsc_zbp_r_online_shop_c32 IMPLEMENTATION.

  METHOD save_modified.
    DATA : lt_online_shop_as TYPE STANDARD TABLE OF zdt_shop_as_c32,
           ls_online_shop_as TYPE                   zdt_shop_as_c32.
    IF zbp_r_online_shop_c32=>cv_pr_mapped-purchaserequisition IS NOT INITIAL.
      LOOP AT zbp_r_online_shop_c32=>cv_pr_mapped-purchaserequisition ASSIGNING FIELD-SYMBOL(<fs_pr_mapped>).
    ...
    IF create-online_shop IS NOT INITIAL.
      ...
      INSERT zdt_shop_as_c32 FROM TABLE @lt_online_shop_as.
    ENDIF.
  ENDMETHOD.
ENDCLASS.

CLASS lhc_zbp_r_online_shop_c32  DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
...
CLASS lhc_zbp_r_online_shop_c32  IMPLEMENTATION.
...
TRY.
      ...
      WITH VALUE #( ( %cid_ref = 'My%ItemCID_1'
                      %target  = VALUE #( (
                                          ...
                                          textobjecttype = 'B01'
                                          language       = 'E'
                                          plainlongtext  = 'item text created from PAAS API c32 - Workshop prep.'
                                        ) (
                                          %cid           = 'My%CCT_2'
                                          textobjecttype = 'B02'
                                          language       = 'E'
                                          plainlongtext  = 'item2 text created from PAAS API c32'
                                        ) )
              )   )
      ...
      zbp_r_online_shop_c32=>cv_pr_mapped = ls_pr_mapped.
...
DATA:
      online_shops TYPE TABLE FOR UPDATE zr_online_shop_c32,
      online_shop  TYPE STRUCTURE FOR UPDATE zr_online_shop_c32.

    READ ENTITIES OF zr_online_shop_c32 IN LOCAL MODE
       ENTITY Online_Shop
        ALL FIELDS
    ...
    LOOP AT lt_online_shop_result INTO DATA(online_shop_read).
      ...
      ENDIF.
    ENDLOOP.
    MODIFY ENTITIES OF zr_online_shop_c32 IN LOCAL MODE
    ENTITY Online_Shop UPDATE SET FIELDS WITH online_shops
    MAPPED   DATA(ls_mapped_modify)
    FAILED   DATA(lt_failed_modify)
    REPORTED DATA(lt_reported_modify).

    DATA: lt_create_pr_imp TYPE TABLE FOR ACTION IMPORT  zr_online_shop_c32~create_pr,
